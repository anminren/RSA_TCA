        ...
        for channel in channels:
            for timepoint in timepoints:
                for factuality in factualitys:
                    spearman_data = []
                    choose_jud = np.load(f'../factuality_significance/{model_name}_{factuality}_{timepoint}_{channel}.npy')
                    # load spearman data from every subjects
                    for i in range(1,14):
                        sp_data = np.load(f'../spearman_corr/spearman/{model_name}_{factuality}_{timepoint}time_course_spearman_corr_{channel}_{i}.npy')
                    ...
                    result[f'{model_name}_{factuality}_{timepoint}_{channel}'] = int(ind[0][0])
    print(result)
    # save result in json file
    with open('./factuality_model_choose.json', 'w') as json_file:
...
def choose_plot_draw():
    ...
    with open('./factuality_model_choose.json', 'r') as file:
        ...
        for factuality in factualitys:
            ...
            for model_name in model_names:
                for channel in channels:
                    ...
                    ind = int(choose_result[f'{model_name}_{factuality}_{timepoint}_{channel}'])
                    if channel == 'Overall':
            ...
            plt.title(f'{factuality} RSA Score in {timepoint}point')
            plt.xlabel('Time')
            plt.ylabel('RSA Score')

            plt.legend()
            plt.savefig(f'../plots/{factuality}_{timepoint}points_plot.png', dpi=300)
            plt.show()
