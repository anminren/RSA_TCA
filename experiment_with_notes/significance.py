...
def plot_significance(result, model_name, channel, timepoint):
    ...
    plt.title(f'{model_name}_{timepoint}_{channel} Significance across Layers and Time Points')
    plt.legend()
    plt.grid(True)
    plt.ylim(0, layers + 1)
    ...
    plt.savefig(f'../significance/{model_name}_{timepoint}_{channel}.png', dpi=300)
    print(f"Plot saved in ../significance/{model_name}_{timepoint}_{channel}.png")

    plt.show()

def significance(model_name, channel, timepoint):
    folder_path = '../spearman_corr/spearman'
    file_pattern = f'{model_name}_{timepoint}time_course_spearman_corr_{channel}_*.npy'
    output_file = f'../significance/{model_name}_{timepoint}_{channel}.npy'

    # Process the file and save the result
    result = calculate_significance(folder_path, file_pattern, output_file)

    # Draw plots
    plot_significance(result[::-1], model_name, channel, timepoint)

def all_run():
    for model_name in model_names:
        for channel in channels:
            for timepoint in timepoints:
                significance(model_name, channel, timepoint)